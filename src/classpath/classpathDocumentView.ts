...
class MarkdownPreviewProvider implements vscode.Disposable {
    ...
    public async show(markdownFilePath: string, title: string, context: vscode.ExtensionContext): Promise<void> {
        if (!this.panel) {
        ...
        this.panel.webview.html = await this.getHtmlContent(this.panel.webview, markdownFilePath, context);
    ...
    protected async getHtmlContent(webview: vscode.Webview, markdownFilePath: string, context: vscode.ExtensionContext): Promise<string> {
